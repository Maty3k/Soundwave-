...
- **Stations.** On another phone or laptop, open the same page and choose **Use this device as a station**, give it a name such as "Kitchen", and leave it in another room. On your main phone, open **Stations**, tap **Add a phone** and let the station scan the QR code (or copy, share and paste the code). The station then shows a reply code for the main phone to scan or paste. While the main phone cannot be reached yet (different networks), the station's reply code refreshes every 25 seconds, so always scan the one currently on its screen; this is what lets two devices on different networks find each other in time. The same Wi-Fi works best. Across different networks the public address lookup lets them find each other, but some mobile networks block the connection. They connect directly to each other (WebRTC, no relay server, no account) and exchange only loudness numbers.
...
src/
  ...
  dsp/             pure signal processing: spectrum, detection, hunting, sound shapes, click maths, radar, listener comparison, test harness
